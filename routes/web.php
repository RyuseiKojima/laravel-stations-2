...
// 詳細ページ（管理者）
Route::get('/admin/schedules/{id}', [ScheduleController::class, 'adminDetail']);
// 作成ページ（管理者）
...
Route::middleware('auth')->group(function () {
  ...
  // Reservation
  // 座席予約ページ
  Route::get('/movies/{movie_id}/schedules/{schedule_id}/reservations/create', [ReservationController::class, 'create'])->middleware(['auth'])->name('reservation.create');
  // 予約
  Route::post('/reservations/store', [ReservationController::class, 'store'])->middleware(['auth'])->name('reservation.store');
});
